...
for i in {1..50};
do
    /opt/mssql-tools/bin/sqlcmd -C -S database -U sa -P $SA_PASSWORD -Q "SELECT 1" > /dev/null 2>&1
    if [ $? -eq 0 ]
    then
...
# Run the SQL script
/opt/mssql-tools/bin/sqlcmd -C -S database -U sa -P $SA_PASSWORD -d master -i /CreateDatabaseAndSeedScripts.sql
